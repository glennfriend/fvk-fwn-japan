...
v0.2.01  --  2014/10/28             --  使用 Imagick() class
...
class magick
{
    ...
    function _run( $command='' )
    {
        ...
        if ('WINNT'==PHP_OS) {
            system(  $path.$command,$out);
        }
        else {
            passthru($path.$command,$out);
        }
     ...
     *  以 高度 為標準限縮
     */
    function resizeHeight( $img, $output, $height )
    {
        $height = intval($height);
        $info = $this->image_info($img); //取得圖檔資訊
        if( $height > $info['height'] || $height<=0 ) {
            $height = $info['height'];
        }

        $obj = new Imagick($img);
        $obj->resizeImage( 0, $height, imagick::FILTER_LANCZOS, 1 );
        $obj->writeImage($output);

        /*
            return $this->_run('convert -geometry x'.$height." $img $output ");
        */
    }
     ...
     *  清析
     */
    function sharpen( $img, $output, $power=1 )
    {
        $power = (int) $power;
        $obj = new Imagick($img);
        $obj->sharpenImage( 1, $power );
        $obj->writeImage($output);
